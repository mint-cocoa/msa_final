    ...
    async def connect(self):
        try:
            ...
            facility_queue = await self.channel.declare_queue(
                "facility_service_queue",
                durable=True
            )
            logger.info("Response queue 'facility_service_queue' declared")
            
            # 토픽 패턴으로 바인딩
            await facility_queue.bind(self.exchange, "facility.response.#")
            logger.info("Response queue bound to 'facility.response.#'")
            
            # 메시지 소비 시작
            await facility_queue.consume(self.process_message)
    ...
    async def process_message(self, message: aio_pika.IncomingMessage):
        async with message.process():
            try:
                ...
                logger.info(f"Response handled successfully: {json.dumps(result, indent=2)}")
