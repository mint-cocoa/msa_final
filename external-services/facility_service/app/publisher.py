...
class EventPublisher:
    ...
    async def create_facility(self, data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """시설 생성 메시지 발행 및 응답 대기"""
        try:
            ...
            await self.client.publish(
                routing_key='facility.request.create',
                message=data,
                correlation_id=correlation_id,
                reply_to="facility.response.create"
            )
            logger.info("Facility creation message published successfully")
                
                
        except Exception as e:
            logger.error(f"시설 생성 메시지 발행 실패: {str(e)}", exc_info=True)
            raise HTTPException(status_code=500, detail=f"시설 생성 실패: {str(e)}")
        finally:
            self._response_event = None

    async def update_facility(self, data: Dict[str, Any]) -> None:
        if not self._connected:
            await self.connect()
        try:
            await self.client.publish('facility.request.update', data)
            logger.info("Facility update message published successfully")
        except Exception as e:
            logger.error(f"시설 업데이트 발행 실패: {str(e)}", exc_info=True)
            raise HTTPException(status_code=500, detail="시설 업데이트 실패")
